...
const SUB_CATEGORY = 'maulana-roohullah-naqshbandi';
const YEAR = 0;
const BASE_URL = 'https://besturdubooks.net/tag/maulana-roohullah-naqshbandi-books/';
const TOTAL_PAGES = 4;
...
async function uploadToDB(books) {
  ...
  for (const book of books) {
    try {
      const dbBook = {
        title: book.title,
        author: 'Maulana Roohullah Naqshbandi',
        category: CATEGORY,
...
async function run() {
  ...
  fs.writeFileSync('naqshbandi_books.json', JSON.stringify(books, null, 2), 'utf8');
  if (books.length > 0) {
