        ...
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                while (scan.length && limit < 40) {
                    ...
                    if (!next.length || next.find('img').length > 0) break;
                ...
                for (const group of publisherGroups) {
                    ...
                    if (resolvedLinks.length > 0) {
                        ...
                        if (bestLinks.length === 0) bestLinks = resolvedLinks;
                        
                        let uniqueVols = [];
                        let urlsSeen = new Set();
                        let maxV = bestLinks.length > 20 ? 40 : bestLinks.length;
...
async function run() {
    console.log("Starting Daura Hadith Urdu Shuroohat Scraping...");
    const data = await scrapeCategoryExpanded('https://besturdubooks.net/dars-e-nizami/dora-e-hadith-books/dora-e-hadith-urdu-shuroohat/', 12, 'اردو شروحات');
    
    fs.writeFileSync('daura_urdu_shuroohat.json', JSON.stringify(data, null, 2));
    console.log(`Finished! Total books scraped: ${data.length}`);
}
