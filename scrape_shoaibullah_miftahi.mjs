...
const SUB_CATEGORY = 'mufti-shoaibullah-miftahi';
const BASE_URL = 'https://besturdubooks.net/tag/mufti-shoaibullah-khan-miftahi-books/';
const TOTAL_PAGES = 6;
...
async function scrapeAll() {
  ...
  for (let i = 0; i < bookUrlsArray.length; i++) {
    ...
    console.log(`📚 [${i + 1}/${bookUrlsArray.length}] Scraping Book: ${bookUrl}`);
    try {
      ...
      const bookData = await page.evaluate(() => {
        ...
        return { title, coverUrl, volumes };
      });

      if (bookData && bookData.title && bookData.volumes.length > 0) {
  ...
  console.log(`\n✅ Total books extracted: ${allBooks.length}\n`);
  allBooks.forEach((book, idx) => {
    console.log(`  ✓ ${book.title}`);
  });

  // Prepare upload data
  const uploadBooks = allBooks.map(book => ({
    title: book.title,
    author: 'Mufti Shoaibullah Khan Miftahi',
    category: CATEGORY,
    sub_category: SUB_CATEGORY,
    cover_url: book.coverUrl,
    pdf_url: book.volumes[0].url,
    description: JSON.stringify({ volumes: book.volumes }),
    is_free: true,
  }));
  ...
  if (error) {
    ...
    console.log(`\n📊 Upload Summary - Successfully uploaded: ${uploadBooks.length}/${uploadBooks.length}`);
