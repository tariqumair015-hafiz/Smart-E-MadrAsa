...
const SUB_CATEGORY = 'maulana-yusuf-ludhianvi';
const YEAR = 0;
const BASE_URL = 'https://besturdubooks.net/tag/maulana-muhammad-yusuf-ludhianvi-books/';
const TOTAL_PAGES = 4;
...
async function scrapeAll() {
  ...
  console.log(`\n🔍 Found total ${bookUrlsArray.length} unique book pages. Beginning deep scrape...`);
  ...
  for (let i = 0; i < bookUrlsArray.length; i++) {
    ...
    try {
        ...
        if (bookData) {
            ...
            console.log(`  ❌ Not enough data found for book (maybe missing cover or links).`);
        }
  ...
  console.log(`\n✅ SCRAPING COMPLETE! Total unique books: ${allBooks.length}`);
  return allBooks;
}
...
async function uploadToDB(books) {
    ...
    for (const book of books) {
      try {
        const dbBook = {
          title: book.title,
          author: 'Maulana Yusuf Ludhianvi',
          category: CATEGORY,
...
async function run() {
    ...
    fs.writeFileSync('yusuf_books.json', JSON.stringify(books, null, 2), 'utf8');
    if (books.length > 0) {
