...
const data = JSON.parse(fs.readFileSync('c:/Users/ut304/.gemini/antigravity/scratch/hafiz-madarsa-pro-v2/sania_books.json', 'utf8'));

async function upload() {
  console.log(`Upserting ${data.length} books for درجہ ثانیہ...`);
  ...
  for (const item of data) {
    ...
    try {
        ...
        if (existing) {
          ...
          // Double check pdf_url to avoid constraint error
          const { data: existingPdf } = await supabase.from('Books').select('id').eq('pdf_url', book.pdf_url).maybeSingle();
          if (existingPdf) {
            console.log(`Skipping duplicate PDF: ${book.title}`);
            continue;
          }
