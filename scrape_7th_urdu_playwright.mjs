...
async function scrapeAll() {
  ...
  for (let pageNum = 1; pageNum <= TOTAL_PAGES; pageNum++) {
    ...
    try {
        ...
        const pageBooks = await page.evaluate(async () => {
            const books = [];
            ...
            for (let img of images) {
                ...
                const processAnchor = (a) => {
                    ...
                    if (href && (href.includes('archive.org/download') || href.includes('archive.org/details') || href.includes('mediafire.com') || href.includes('drive.google.com'))) {
                        ...
                        // Try to ignore mediafire duplicate if we have archive
                ...
                if (volumes.length === 0 && detailPageUrl && detailPageUrl.startsWith('http')) {
                    // Try fetch detail page
                    try {
                        let res = await fetch(detailPageUrl);
                        let html = await res.text();
                        let parser = new DOMParser();
                        let doc = parser.parseFromString(html, 'text/html');
                        
                        Array.from(doc.querySelectorAll('.entry-content a, .post-content a')).forEach(a => {
                         ...
                         console.error("Fetch subpage failed", detailPageUrl);
                    }
                }
