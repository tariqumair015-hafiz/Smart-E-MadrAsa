...
async function scrape() {
        ...
        try {
            ...
            images.each((i, imgEl) => {
                ...
                if (coverUrl.includes('i2.wp.com')) {
                    // example: https://i2.wp.com/besturdubooks.net/wp-content/uploads/2019/09/TADREES_E_QURAN.jpg?resize=283%2C425&ssl=1
                ...
                // If it's still besturdubooks.net URL but has query params
                ...
                while (prev.length && !title) {
                    ...
                    if (text && text.length > 5 && !text.includes('Click here') && !text.includes('Download') && !text.includes('Nisab')) {
                        title = text;
                    }
                    prev = prev.prev();
                }
                ...
                if (title && volumes.length > 0) {
                    ...
                    console.log(`Added: ${title} - Cover: ${coverUrl}`);
                }
            });
    ...
    fs.writeFileSync('sania_books.json', JSON.stringify(allBooks, null, 2));
    console.log('Saved to sania_books.json');
}
