...
async function collectAllSaniaLinks() {
  ...
  for (let page = 1; page <= 9; page++) {
    ...
    try {
      ...
      // Standard WordPress/Kadence structure: <h2 class="entry-title"><a href="...">Title</a></h2>
      const entryRegex = /<h2[^>]*class="[^"]*entry-title[^"]*"[^>]*>\s*<a\s+href="([^"]*)"[^>]*>(.*?)<\/a>/g;
      let match;
      let count = 0;
      ...
      console.log(`  Found ${count} new books on page ${page}. (Total unique: ${allLinks.size})`);
    } catch (e) {
