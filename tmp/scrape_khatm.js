...
async function run() {
  ...
  for (let page = 1; page <= 5; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/khatm-e-nubuwwat/'
      : `https://besturdubooks.net/category/khatm-e-nubuwwat/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'ختم نبوت', // "Khatm e Nubuwwat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Khatm e Nubuwwat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Khatm e Nubuwwat books inserted successfully!');
  }
}
