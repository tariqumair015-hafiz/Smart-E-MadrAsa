...
async function run() {
  ...
  for (let page = 1; page <= 5; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/tableegh/'
      : `https://besturdubooks.net/category/tableegh/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'تبلیغ و دعوت', // "Tableegh"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Tableegh category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Tableegh books inserted successfully!');
  }
}
