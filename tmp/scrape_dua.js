...
async function run() {
  ...
  for (let page = 1; page <= 14; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/doa-o-darood/'
      : `https://besturdubooks.net/category/doa-o-darood/page/${page}/`;

    console.log(`Scraping page ${page}/14: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'عملیات / طب / ادعیہ', // "Dua ow Darood"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Dua ow Darood: ${allBooks.length}`);
  ...
  console.log('🎉 All Dua ow Darood books inserted successfully!');
}
