...
async function run() {
  ...
  for (let page = 1; page <= 22; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/gumrah-firqe/'
      : `https://besturdubooks.net/category/gumrah-firqe/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'فرق ضاله', // "Deviant Sects"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Deviant Sects: ${allBooks.length}`);
