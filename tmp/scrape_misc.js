...
async function run() {
  ...
  for (let page = 1; page <= 69; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/general/'
      : `https://besturdubooks.net/category/general/page/${page}/`;

    console.log(`Scraping page ${page}/69: ${url}...`);
    try {
      ...
      for (const item of uniqueLinks) {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'متفرقات', // "Miscellaneous"
              cover_url: img,
        ...
        await delay(400); // Increased delay slightly for large run
      }
      
      // Batch insert every 5 pages to save progress
      if (page % 5 === 0 && allBooks.length > 0) {
        const toInsert = [...allBooks];
        allBooks.length = 0; // Clear for next batch
        const { error } = await supabase.from('Books').upsert(toInsert, { onConflict: 'pdf_url' });
        if (error) console.error(`Batch insert error at page ${page}:`, error);
  ...
  console.log(`\nTotal books found for Miscellaneous category: `);
  ...
  console.log('🎉 All Miscellaneous books inserted successfully!');
}
