...
async function run(baseUrl, subCat, totalPages) {
        ...
        try {
            ...
            content.find('img').each((i, el) => {
                ...
                if (currentLinks.length > 0) {
                    ...
                    allBooks.push({
                        title: title, author: 'نامعلوم', category: 'دورہ حدیث',
                        sub_category: subCat, cover_url: coverUrl, pdf_url: finalLinks[0].url,
                        description: JSON.stringify(finalLinks), pages: 0, is_free: true, language: 'ur'
                    });
                }
            });
        } catch (e) {
    ...
    else console.log(`Done with ${subCat}. Added/Updated ${uniqueBooks.length} books.`);
}

run('https://besturdubooks.net/dars-e-nizami/dora-e-hadith-books/dora-e-hadith-text-books/', 'درسی کتب', 3);
