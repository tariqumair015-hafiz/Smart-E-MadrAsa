...
async function run() {
  ...
  for (let page = 1; page <= 23; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/seerat-un-nabi/'
      : `https://besturdubooks.net/category/seerat-un-nabi/page/${page}/`;

    console.log(`Scraping page ${page}/23: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'سیرت و تاریخ', // "Seerat & History"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Seerat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Seerat books inserted successfully!');
  }
}
