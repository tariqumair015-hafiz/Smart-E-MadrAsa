...
async function run() {
  ...
  for (let page = 1; page <= 20; page++) {
    const url = page === 1
      ...
      : `https://besturdubooks.net/category/khutbaat-o-maqalaat/page/${page}/`;

    console.log(`Scraping page ${page}/20: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'خطبات / مقالات / محاضرات', // "Khutbaat o Maqalaat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Khutbaat o Maqalaat: ${allBooks.length}`);
  ...
  console.log('🎉 All Khutbaat o Maqalaat books inserted successfully!');
}
