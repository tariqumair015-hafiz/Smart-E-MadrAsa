...
async function run() {
  ...
  for (let page = 1; page <= 7; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/encyclopedias/'
      : `https://besturdubooks.net/category/encyclopedias/page/${page}/`;

    console.log(`Scraping page ${page}/7: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'دائرۃ المعارف', // "Encyclopedias"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Encyclopedias: ${allBooks.length}`);

  if (allBooks.length > 0) {
    const batchSize = 50;
    for (let i = 0; i < allBooks.length; i += batchSize) {
      const batch = allBooks.slice(i, i + batchSize);
      const { data: upsertData, error } = await supabase.from('Books').upsert(batch, { onConflict: 'pdf_url' }).select('id, category');
      if (error) {
        console.error(`Supabase error on batch ${Math.floor(i/batchSize)+1}:`, error);
      } else {
        console.log(`✅ Batch ${Math.floor(i/batchSize)+1} processed (${batch.length} books)`);
        console.log(`Sample upserted category: ${upsertData[0]?.category}`);
      }
    }
    console.log('🎉 All Encyclopedias books inserted successfully!');
  }
}
