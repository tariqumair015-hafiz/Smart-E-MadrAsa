...
async function run() {
  ...
  for (let page = 1; page <= 19; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/jadid-fiqh/'
      : `https://besturdubooks.net/category/jadid-fiqh/page/${page}/`;

    console.log(`Scraping page ${page}/19: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'جدید فقہ', // "Modern Jurisprudence"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Modern Jurisprudence: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Modern Jurisprudence books inserted successfully!');
  }
}
