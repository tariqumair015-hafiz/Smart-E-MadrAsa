...
async function run() {
  ...
  for (let page = 1; page <= 7; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/children/'
      : `https://besturdubooks.net/category/children/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'بچوں کی کتب', // Matches App.jsx label 'بچوں کی کتب'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Children Books: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Children Books inserted successfully!');
  }
}
