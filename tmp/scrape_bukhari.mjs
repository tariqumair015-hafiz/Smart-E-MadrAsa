...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/hafiz-muhammad-akbar-shah-bukhari-books';
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('hafiz-muhammad-akbar-shah-bukhari-books')) {
  ...
  // Filter out known non-book links
  const finalLinks = linksData.filter(l => {
    // Keep it broad unless we know it's failing
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Hafiz Muhammad Akbar Shah Bukhari",
          category: 'علمائے کرام',
          sub_category: 'hafiz-akbar-shah-bukhari',
          cover_url: bookImg,
