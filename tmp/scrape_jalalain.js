...
async function run() {
  ...
  for (let page = 1; page <= 2; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/sharah-tafsir-jalalain/'
      : `https://besturdubooks.net/tag/sharah-tafsir-jalalain/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'جلالین کی شروحات', // "Jalalain Sharuhat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Jalalain Sharuhat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Jalalain Sharuhat books inserted successfully!');
  }
}
