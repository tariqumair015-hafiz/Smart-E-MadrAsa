...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/maulana-muhammad-manzoor-nomani-books';
  const pages = ['', '/page/2/'];
  const linksData = [];
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('maulana-muhammad-manzoor-nomani-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Manzoor Nomani",
          category: 'علمائے کرام',
          sub_category: 'maulana-manzoor-nomani',
          cover_url: bookImg,
