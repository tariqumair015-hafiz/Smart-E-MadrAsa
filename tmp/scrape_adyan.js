...
async function run() {
  ...
  for (let page = 1; page <= 6; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/taqabul-e-adyan/'
      : `https://besturdubooks.net/category/taqabul-e-adyan/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'تقابل ادیان', // "Taqabul e Adyan"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Taqabul e Adyan category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Taqabul e Adyan books inserted successfully!');
  }
}
