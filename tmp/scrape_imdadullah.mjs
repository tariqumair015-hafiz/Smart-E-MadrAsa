...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/maulana-imdadullah-anwar-books';
  const pages = ['', '/page/2/', '/page/3/', '/page/4/'];
  const linksData = [];
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('maulana-imdadullah-anwar-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Imdadullah Anwar",
          category: 'علمائے کرام',
          sub_category: 'maulana-imdadullah-anwar',
          cover_url: bookImg,
