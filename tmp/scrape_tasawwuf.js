...
async function run() {
  ...
  for (let page = 1; page <= 21; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/tasawwuf/'
      : `https://besturdubooks.net/category/tasawwuf/page/${page}/`;

    console.log(`Scraping page ${page}/21: ${url}...`);
    try {
  ...
  console.log(`\nTotal books found for Tasawwuf category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Tasawwuf books inserted successfully!');
  }
}
