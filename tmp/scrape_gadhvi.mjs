...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/muhammad-ilyas-bin-abdullah-books';
  const pages = ['', '/page/2/'];
  const linksData = [];
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('muhammad-ilyas-bin-abdullah-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Ilyas Abdullah Gadhvi",
          category: 'علمائے کرام',
          sub_category: 'maulana-ilyas-abdullah-gadhvi',
          cover_url: bookImg,
