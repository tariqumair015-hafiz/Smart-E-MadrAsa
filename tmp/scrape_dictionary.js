...
async function run() {
  ...
  for (let page = 1; page <= 7; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/dictionary/'
      : `https://besturdubooks.net/category/dictionary/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'لغت', // "Dictionary / Dictionaries" — matches App.jsx label
              cover_url: img,
  ...
  console.log(`\nTotal books found for Dictionary category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Dictionary books inserted successfully!');
  }
}
