...
async function run() {
  ...
  for (let page = 1; page <= 12; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/khawateen/'
      : `https://besturdubooks.net/category/khawateen/page/${page}/`;

    console.log(`Scraping page ${page}/12: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'خواتین', // Matches App.jsx label 'خواتین'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Khawateen: ${allBooks.length}`);
