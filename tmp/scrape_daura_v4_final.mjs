...
async function runScraper() {
    const baseUrl = 'https://besturdubooks.net/dars-e-nizami/dora-e-hadith-books/dora-e-hadith-text-books/';
    const totalPages = 3;
    const allBooks = [];
    ...
    for (let page = 1; page <= totalPages; page++) {
        ...
        try {
            ...
            content.find('img').each((i, el) => {
                ...
                // If no title, try preceding text
                if (!title) {
                    let prev = img.parent().prev();
                    if (prev.length) title = prev.text().trim();
                }
                if (!title) title = `Daura Hadith Book ${allBooks.length + 1}`;
                ...
                // Scan inside and after the image block
                scan.find('a').each((j, a) => processAnchor(a));
                while (scan.length && limit < 15) {
                    scan = scan.next();
                    if (!scan.length || scan.find('img').length > 0) break; // Next book started
                    scan.find('a').each((j, a) => processAnchor(a));
                    limit++;
                }

                if (currentLinks.length > 0) {
                    // Filter out duplicates (mediafire vs archive mirrors for same vol)
                    let finalLinks = [];
        ...
        await delay(3000); // Respect the server
    }
    ...
    if (allBooks.length > 0) {
        ...
        else console.log('Successfully updated Daura Hadith textbooks!');
    }
}
