...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/maulana-ijaz-ahmad-azmi-books';
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('maulana-ijaz-ahmad-azmi-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Ijaz Ahmad Azmi",
          category: 'علمائے کرام',
          sub_category: 'maulana-ijaz-ahmad-azmi',
          cover_url: bookImg,
