...
async function run() {
  ...
  for (let page = 1; page <= 6; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/ahkam-o-masail/hajj-o-umrah/'
      : `https://besturdubooks.net/category/ahkam-o-masail/hajj-o-umrah/page/${page}/`;
  ...
  console.log(`\nTotal books found for Hajj & Umrah: ${allBooks.length}`);

  if (allBooks.length > 0) {
    const batchSize = 50;
    for (let i = 0; i < allBooks.length; i += batchSize) {
      const batch = allBooks.slice(i, i + batchSize);
      // Upsert without ignoreDuplicates to update the category for existing books
    ...
    console.log('🎉 All Hajj & Umrah books inserted successfully!');
  }
}
