...
async function run() {
  ...
  for (let page = 1; page <= 9; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/sharah-sahih-ul-bukhari/'
      : `https://besturdubooks.net/tag/sharah-sahih-ul-bukhari/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'بخاری شریف کی شروحات', // "Bukhari Sharuhat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Bukhari Sharuhat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Bukhari Sharuhat books inserted successfully!');
  }
}
