...
async function run() {
  ...
  for (let page = 1; page <= 3; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/al-aqeedah-al-tahawiyyah-book/'
      : `https://besturdubooks.net/tag/al-aqeedah-al-tahawiyyah-book/page/${page}/`;

    console.log(`Scraping page ${page}/3: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'عقیدہ طحاویہ مع شروحات', // Matches App.jsx label 'عقیدہ طحاویہ مع شروحات'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Al Aqeedah Al Tahawiyyah: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 Al Aqeedah Al Tahawiyyah books inserted successfully!');
  }
}
