...
async function run() {
  ...
  for (let page = 1; page <= 6; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/sharah-al-tirmizi/'
      : `https://besturdubooks.net/tag/sharah-al-tirmizi/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'ترمذی شریف کی شروحات', // "Tirmidhi Sharuhat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Tirmidhi Sharuhat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Tirmidhi Sharuhat books inserted successfully!');
  }
}
