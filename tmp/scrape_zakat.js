...
async function run() {
  ...
  for (let page = 1; page <= 3; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/ahkam-o-masail/zakat/'
      : `https://besturdubooks.net/category/ahkam-o-masail/zakat/page/${page}/`;
  ...
  console.log(`\nTotal books found for Zakat: ${allBooks.length}`);

  if (allBooks.length > 0) {
    const batchSize = 50;
    for (let i = 0; i < allBooks.length; i += batchSize) {
      const batch = allBooks.slice(i, i + batchSize);
      // Removed ignoreDuplicates so it updates the category if the book already exists from the parent category 'Ahkam & Masail'
    ...
    console.log('🎉 All Zakat books inserted successfully!');
  }
}
