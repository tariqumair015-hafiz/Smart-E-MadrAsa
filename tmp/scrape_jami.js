...
async function run() {
  ...
  for (let page = 1; page <= 4; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/sharah-jami-book/'
      : `https://besturdubooks.net/tag/sharah-jami-book/page/${page}/`;

    console.log(`Scraping page ${page}/4: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'شرح جامی مع شروحات', // Matches App.jsx label 'شرح جامی مع شروحات'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Sharah Jami: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 Sharah Jami books inserted successfully!');
  }
}
