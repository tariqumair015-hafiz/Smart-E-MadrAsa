...
async function run() {
  ...
  for (let page = 1; page <= 2; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/noor-ul-anwaar-book/'
      : `https://besturdubooks.net/tag/noor-ul-anwaar-book/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'نور الانوار مع شروحات', // Matches App.jsx label 'نور الانوار مع شروحات'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Noor ul Anwaar: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 Noor ul Anwaar books inserted successfully!');
  }
}
