...
async function run() {
  ...
  for (let page = 1; page <= 4; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/sharah-al-hidayah-book/'
      : `https://besturdubooks.net/tag/sharah-al-hidayah-book/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'ہدایہ کی شروحات', // "Hedaya Sharuhat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Hedaya Sharuhat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Hedaya Sharuhat books inserted successfully!');
  }
}
