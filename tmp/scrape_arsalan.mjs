...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/maulana-arsalan-bin-akhtar-books';
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('maulana-arsalan-bin-akhtar-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Arsalan Bin Akhtar",
          category: 'علمائے کرام',
          sub_category: 'maulana-arsalan-bin-akhtar',
          cover_url: bookImg,
