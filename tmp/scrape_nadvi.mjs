...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/allama-abul-hasan-ali-nadwi-books';
  const pages = ['', '/page/2/', '/page/3/', '/page/4/', '/page/5/', '/page/6/'];
  const linksData = [];
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('allama-abul-hasan-ali-nadwi-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Abul Hasan Ali Nadvi",
          category: 'علمائے کرام',
          sub_category: 'maulana-abul-hasan-ali-nadvi',
          cover_url: bookImg,
    ...
    // Slight delay to be nice to the server since we are scraping ~60 books
    await delay(200);
  }
