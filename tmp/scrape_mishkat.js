...
async function run() {
  ...
  for (let page = 1; page <= 5; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/sharah-mishkat-al-masabih/'
      : `https://besturdubooks.net/tag/sharah-mishkat-al-masabih/page/${page}/`;

    console.log(`Scraping page ${page}/5: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'مشکوٰۃ شریف کی شروحات', // "Mishkat Sharuhat"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Mishkat Sharuhat category: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Mishkat Sharuhat books inserted successfully!');
  }
}
