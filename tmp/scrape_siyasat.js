...
async function run() {
  ...
  for (let page = 1; page <= 8; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/siyasat/'
      : `https://besturdubooks.net/category/siyasat/page/${page}/`;

    console.log(`Scraping page ${page}/8: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'سیاست', // Matches App.jsx
              cover_url: img,
  ...
  console.log(`\nTotal books found for Siyasat: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Siyasat books inserted successfully!');
  }
}
