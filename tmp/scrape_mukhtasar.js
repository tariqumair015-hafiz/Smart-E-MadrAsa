...
async function run() {
  ...
  for (let page = 1; page <= 2; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/mukhtasar-ul-maani-book/'
      : `https://besturdubooks.net/tag/mukhtasar-ul-maani-book/page/${page}/`;

    console.log(`Scraping page ${page}/2: ${url}...`);
    try {
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'مختصر المعانی مع شروحات', // Matches App.jsx label 'مختصر المعانی مع شروحات'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Mukhtasar ul Maani: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 Mukhtasar ul Maani books inserted successfully!');
  }
}
