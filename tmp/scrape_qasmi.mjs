...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/maulana-ala-ud-din-qasmi-books';
  const pages = ['', '/page/2/', '/page/3/', '/page/4/', '/page/5/', '/page/6/', '/page/7/'];
  const linksData = [];
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('maulana-ala-ud-din-qasmi-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Ala ud Din Qasmi",
          category: 'علمائے کرام',
          sub_category: 'maulana-ala-ud-din-qasmi',
          cover_url: bookImg,
    ...
    // Batch Insert logic in case of 40-50+ books
