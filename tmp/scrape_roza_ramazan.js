...
async function run() {
  ...
  for (let page = 1; page <= 9; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/ahkam-o-masail/roza-ramazan/'
      : `https://besturdubooks.net/category/ahkam-o-masail/roza-ramazan/page/${page}/`;

    console.log(`Scraping page ${page}/9: ${url}...`);
    try {
      ...
      for (const item of uniqueLinks) {
        ...
        try {
          ...
          $2('a').each((i, link) => {
            ...
            if (h && (h.toLowerCase().includes('.pdf') || h.toLowerCase().includes('download'))) {
              if (h.toLowerCase().includes('.pdf')) {
                ...
                // Check if it looks like a volume link
                volumes.push({
                   title: linkText || `جلد ${volumes.length + 1}`,
                   url: h
                });
          ...
          // Some cleanup for volumes - remove duplicates and non-pdf/download junk
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'روزہ', // "Fasting (Roza)"
              cover_url: img,
              pdf_url: volumes[0].url,
              description: JSON.stringify(volumes), // Store all volumes as JSON
              pages: 0,
              is_free: true,
              downloads: 0,
              rating: 0,
            ...
            console.log(`    ✗ Skipped: ${title} (no pdf/img)`);
          }
        } catch (e) {
  ...
  console.log(`\nTotal books found for Roza & Ramazan: ${allBooks.length}`);

  if (allBooks.length > 0) {
    const batchSize = 50;
    for (let i = 0; i < allBooks.length; i += batchSize) {
      const batch = allBooks.slice(i, i + batchSize);
      const { error } = await supabase.from('Books').upsert(batch, { onConflict: 'pdf_url', ignoreDuplicates: false });
      if (error) {
    ...
    console.log('🎉 All Roza & Ramazan books inserted successfully!');
  }
}
