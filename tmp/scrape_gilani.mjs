...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/maulana-manazir-ahsan-gilani-books';
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            !href.includes('maulana-manazir-ahsan-gilani-books')) {
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (volumes.length > 0) {
        allBooks.push({
          title: title,
          author: "Maulana Manazir Ahsan Gilani",
          category: 'علمائے کرام',
          sub_category: 'maulana-manazir-ahsan-gilani',
          cover_url: bookImg,
