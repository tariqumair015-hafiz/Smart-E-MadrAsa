...
async function run() {
  const baseUrl = 'https://besturdubooks.net/tag/dr-mahmood-ahmad-ghazi-books';
    ...
    try {
      ...
      $('a').each((i, el) => {
        ...
        if (href && href.startsWith('https://besturdubooks.net/') && 
            ...
            href !== 'https://besturdubooks.net/tag/dr-mahmood-ahmad-ghazi-books/') {
                
            if (href.length > 35) { // usually book URL are long
                if (!linksData.some(l => l.href === href)) {
                    // Only push if the url slug specifically looks like a book related to him
  ...
  // Filter out any potential non-book links
  const finalLinks = linksData.filter(l => {
    return l.href.includes('muhazarat') || 
           l.href.includes('islam-ka-qanoon') || 
           l.href.includes('quran') || 
           l.href.includes('muhazirat') ||
           l.href.includes('paighambar') ||
           l.href.includes('ahkam') ||
           l.href.includes('ghazi') ||
           l.href.includes('taleem');
  });
  ...
  for (let i = 0; i < finalLinks.length; i++) {
    ...
    try {
      ...
      if (!title || title.toLowerCase() === 'open' || title.toLowerCase() === 'read online' || title.includes("محاضرات") || title.trim() === '') {
          // Extra cleanup for titles
      ...
      // Attempt to pick a cover that isn't a known ad placeholder
      ...
      if (volumes.length > 0) {
        allBooks.push({
          ...
          sub_category: 'dr-mahmood-ahmad-ghazi',
          cover_url: bookImg,
