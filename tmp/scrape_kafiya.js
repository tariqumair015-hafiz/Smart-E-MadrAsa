...
async function run() {
  ...
  for (let page = 1; page <= 3; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/tag/kafiya-ibn-e-hajib-book/'
      : `https://besturdubooks.net/tag/kafiya-ibn-e-hajib-book/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'کافیہ مع شروحات', // Matches App.jsx label 'کافیہ مع شروحات'
              cover_url: img,
  ...
  console.log(`\nTotal books found for Kafiya: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 Kafiya books inserted successfully!');
  }
}
