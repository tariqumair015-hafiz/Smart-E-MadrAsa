...
async function run() {
  ...
  for (let page = 1; page <= 2; page++) {
    const url = page === 1
      ? 'https://besturdubooks.net/category/khwaab-o-tabeer/'
      : `https://besturdubooks.net/category/khwaab-o-tabeer/page/${page}/`;
        ...
        try {
          ...
          if (volumes.length > 0 && img && title) {
            allBooks.push({
              title,
              author: 'نامعلوم',
              category: 'خواب و تعبیر', // "Dream Interpretation"
              cover_url: img,
  ...
  console.log(`\nTotal books found for Dream Interpretation: ${allBooks.length}`);
  ...
  if (allBooks.length > 0) {
    ...
    console.log('🎉 All Dream Interpretation books inserted successfully!');
  }
}
