...
const SUB_CATEGORY = 'maulana-qari-tayyab';
const YEAR = 0;
const BASE_URL = 'https://besturdubooks.net/tag/maulana-qari-muhammad-tayyab-books/';
const TOTAL_PAGES = 5;
...
async function uploadToDB(books) {
  ...
  for (const book of books) {
    try {
      const dbBook = {
        title: book.title,
        author: 'Maulana Qari Tayyab Qasmi',
        category: CATEGORY,
...
async function run() {
  ...
  fs.writeFileSync('tayyab_books.json', JSON.stringify(books, null, 2), 'utf8');
  if (books.length > 0) {
