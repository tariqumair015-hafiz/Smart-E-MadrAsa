...
(async () => {
  console.log('📋 Verifying Zulfiqar Naqshbandi books upload...\n');

  const { data, error } = await supabase
    .from('Books')
    .select('title, author, cover_url, pdf_url')
    .eq('sub_category', 'maulana-zulfiqar-naqshbandi');
  ...
  data.forEach((book, idx) => {
    ...
    if (hasValidCover) {
      validCovers++;
      console.log(`✓ [${idx + 1}/${data.length}] ${book.title.substring(0, 50)}`);
    } else {
      invalidCovers++;
