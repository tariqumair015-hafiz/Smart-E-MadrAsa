...
const SUB_CATEGORY = 'maulana-idrees-kandhalvi';
const YEAR = 0;
const BASE_URL = 'https://besturdubooks.net/tag/maulana-muhammad-idrees-kandhalvi-books/';
const TOTAL_PAGES = 3;
  ...
  for (let i = 0; i < bookUrlsArray.length; i++) {
    ...
    try {
      ...
      const bookData = await page.evaluate(() => {
        ...
        // Volumes / download links
        const volumes = [];
        const links = Array.from(content.querySelectorAll('a'));
        links.forEach(a => {
          const href = a.getAttribute('href') || '';
          const text = a.textContent.trim().toLowerCase();
          if (text.includes('link 2') || text.includes('link 02') || text.includes('read online') || text.includes('link 3')) return;
...
async function uploadToDB(books) {
  ...
  for (const book of books) {
    try {
      const dbBook = {
        title: book.title,
        author: 'Maulana Idrees Kandhalvi',
        category: CATEGORY,
...
async function run() {
  ...
  fs.writeFileSync('idrees_books.json', JSON.stringify(books, null, 2), 'utf8');
  if (books.length > 0) {
